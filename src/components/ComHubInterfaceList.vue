<template>
    <div>
        <div
            ...
            :key="iface.uuid"
            class="card cursor-pointer transition hover:bg-neutral-100/80 dark:hover:bg-neutral-800/80"
        >
            <!-- Interface Header -->
            <div class="flex items-start justify-between gap-2">
                <div class="flex-1 cursor-pointer" @click="toggle(iface.uuid)">
                    <h3 class="text-primary font-semibold">
                        {{ iface.properties?.interface_type || 'Unknown interface' }}
                        <span v-if="iface.properties && iface.properties.name"
                            >({{ iface.properties.name }})</span
                        >
                    </h3>
                    <div class="text-dim mt-1 text-xs">
                        Sockets: {{ iface.sockets.length }} • Channel:
                        {{ iface.properties?.channel ?? 'unknown' }}
                    </div>
                </div>
                <div class="flex items-center gap-2">
                    <div class="text-faint text-xs">
                        RTT: {{ iface.properties?.round_trip_time ?? 'N/A' }} ms
                    </div>
                    <button
                        v-if="advancedMode"
                        @click.stop="disconnectInterface(iface.uuid)"
                        class="btn-danger mr-3 w-24 text-center"
                    >
                        Disconnect
                    </button>
                </div>
            ...
            <div v-if="expanded[iface.uuid]" class="mt-3 flex flex-col gap-3 border-t pt-3">
                <div
        ...
        <div v-if="filteredInterfaces.length === 0" class="text-dim text-sm">
