...
const tree: Ref<NodeTree<unknown, unknown>> = ref(props.initialTree ?? parseNodeTree(example));
...
// Edge being created
const pendingEdge = ref<{ sourceFieldId: string; sourceNodeId: string; isOut: boolean } | null>(
    null,
);
...
function completeEdge(targetFieldId: string, targetNodeId: string) {
    ...
    if (sourceField.connectors?.length) {
        ...
        if (maxConnections !== undefined) {
            const existingCount = tree.value.edges.filter(
                (e) =>
                    e.source.kind === 'field' &&
                    e.source.fieldId === pendingEdge.value!.sourceFieldId,
            ).length;
    ...
    if (targetField.connectors?.length) {
        ...
        if (maxConnections !== undefined) {
            const existingCount = tree.value.edges.filter(
                (e) => e.target.kind === 'field' && e.target.fieldId === targetFieldId,
            ).length;
            if (existingCount >= maxConnections) {
    ...
    const newEdge: Edge = {
        ...
        source: {
            kind: 'field',
            fieldId: pendingEdge.value.sourceFieldId,
            nodeId: pendingEdge.value.sourceNodeId,
        },
...
const draggedNodeEdges = computed(() => {
    ...
    return tree.value.edges.filter(
        (e) =>
...
function resolveCollisions() {
    ...
    for (let i = 0; i < len; i++) {
        for (let j = i + 1; j < len; j++) {
            ...
            const overlapY =
                (aHeight + bHeight) / 2 + padding - Math.abs(nodeA.position.y - nodeB.position.y);
    ...
    if (event.touches.length === 1) {
        ...
        if (nodeId) {
            ...
            if (node) {
                startPos.value = {
                    x:
                        (touch.clientX - rect.left - panOffset.value.x) / scale.value -
                        node.position.x,
                    y:
                        (touch.clientY - rect.top - panOffset.value.y) / scale.value -
                        node.position.y,
                };
            }
        } else {
...
async function handleKeydown(event: KeyboardEvent) {
    ...
    if (cmdOrCtrl && event.key === 'x' && activeNodeIds.value.size > 0) {
        ...
        try {
            await navigator.clipboard.writeText(
                JSON.stringify({ nodes: cutNodes, edges: cutEdges }),
            );
        } catch (err) {
            console.error('[NodeTree] Failed to write to clipboard', err);
        }
    ...
    if (cmdOrCtrl && event.key === 'v') {
        ...
        try {
            ...
            const subtree = JSON.parse(text) as {
                nodes: NodeTree['nodes'];
                edges: NodeTree['edges'];
            };
            ...
            const newEdges = subtree.edges.map((edge) => {
                const newSource =
                    edge.source.kind === 'field'
                        ? {
                              kind: 'field' as const,
                              nodeId: nodeIdMap.get(edge.source.nodeId) ?? edge.source.nodeId,
                              fieldId: fieldIdMap.get(edge.source.fieldId) ?? edge.source.fieldId,
                          }
                        : {
                              kind: 'node' as const,
                              nodeId: nodeIdMap.get(edge.source.nodeId) ?? edge.source.nodeId,
                ...
                const newTarget =
                    edge.target.kind === 'field'
                        ? {
                              kind: 'field' as const,
                              nodeId: nodeIdMap.get(edge.target.nodeId) ?? edge.target.nodeId,
                              fieldId: fieldIdMap.get(edge.target.fieldId) ?? edge.target.fieldId,
                          }
                        : {
                              kind: 'node' as const,
                              nodeId: nodeIdMap.get(edge.target.nodeId) ?? edge.target.nodeId,
...
<template>
    ...
    >
        ...
        <!-- Hidden file input -->
        <input ref="fileInputRef" type="file" accept=".json" class="hidden" @change="importTree" />
        ...
        <div class="top-offset absolute top-0 left-0 w-full flex justify-between items-center p-2">
            ...
            <div class="top-3 right-3 z-50 flex gap-2">
                ...
                >
                    ...
                    <svg
                        v-else-if="backgroundPattern === 'checkerboard'"
                        xmlns="http://www.w3.org/2000/svg"
                        class="w-4 h-4"
                        viewBox="0 0 24 24"
                        fill="currentColor"
